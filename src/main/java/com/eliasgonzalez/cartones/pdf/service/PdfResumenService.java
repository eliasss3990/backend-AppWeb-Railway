...
import com.eliasgonzalez.cartones.pdf.enums.ColumnsResumenEnum;
...
public class PdfResumenService {
    ...
    static {
        POS_COLUMNAS.put(ColumnsResumenEnum.VENDEDOR.getValue(), MARGEN + 15);
        POS_COLUMNAS.put(ColumnsResumenEnum.S_DEL.getValue(), MARGEN + 260);
        POS_COLUMNAS.put(ColumnsResumenEnum.S_AL.getValue(), MARGEN + 310);
        POS_COLUMNAS.put(ColumnsResumenEnum.T_DEL.getValue(), MARGEN + 360);
        POS_COLUMNAS.put(ColumnsResumenEnum.T_AL.getValue(), MARGEN + 410);
        POS_COLUMNAS.put(ColumnsResumenEnum.CANTIDAD_S.getValue(), MARGEN + 460);
        POS_COLUMNAS.put(ColumnsResumenEnum.CANTIDAD_T.getValue(), MARGEN + 510);
    }
    ...
    public byte[] generarResumen(List<ResumenDTO> vendedores, String fechaPersonalizada) throws Exception {
        ...
        float xInicioSenete = POS_COLUMNAS.get(ColumnsResumenEnum.CANTIDAD_S.getValue());
        float xInicioTelebingo = POS_COLUMNAS.get(ColumnsResumenEnum.CANTIDAD_T.getValue());
...
//    }
    ...
    private void dibujarEncabezados(PdfContentByte cb, float y, float width, BaseFont font) {
        ...
        float xSDel = POS_COLUMNAS.get(ColumnsResumenEnum.S_DEL.getValue());
        float xSAl = POS_COLUMNAS.get(ColumnsResumenEnum.S_AL.getValue());
        float centerSeneteGroup = (xSDel + xSAl) / 2; // Centro del grupo Seneté Rangos

        float xTDel = POS_COLUMNAS.get(ColumnsResumenEnum.T_DEL.getValue());
        float xTAl = POS_COLUMNAS.get(ColumnsResumenEnum.T_AL.getValue());
